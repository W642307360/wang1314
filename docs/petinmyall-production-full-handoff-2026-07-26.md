...
## 24. 2026-07-30 数据库容量、安全删除与前端稳定性升级

### 24.1 当前生产版本

- 发布版本：`20260730-product-purge-scale-v1`
- 数据库迁移：`045_product_purge_and_scale_indexes.sql`
- 发布后数据库：`integrity_check=ok`
- 发布后外键异常：`0`
- 发布前后核心计数一致：579用户、201商品、44订单、5支付、8物流事件、49客服会话、286消息

### 24.2 商品删除规则

- 管理后台新增“删除并释放空间”。
- 没有订单或未知业务引用时：
  - 在单个SQLite事务内清理允许删除的收藏、足迹、库存、商品销售记录等从属数据。
  - 删除商品主记录，依靠外键级联清理图片、视频、SKU、评价和身份证档案。
  - 事务提交后才处理本地文件，数据库回滚时不会提前删除硬盘文件。
- 存在订单或其他不可丢失业务引用时：
  - 不物理删除商品、不删除媒体。
  - 自动改为下架归档。
  - 保留订单快照、支付、物流、售后和历史记录。
- 本地媒体删除前会扫描数据库全部文本字段：
  - 其他商品或业务仍引用的共享文件必须保留。
  - 外部URL和COS/CDN地址不作为本地文件删除。
  - 只允许删除解析后严格位于 `server/uploads` 内的普通文件。
  - 拒绝路径穿越、目录、符号链接和上传根目录删除。
- 删除过程记录在：
...
- 使用独立临时SQLite数据库。
- 生成10000条商品文字记录、10000条图片URL元数据、10000条视频URL元数据和10000条库存记录。
- 不生成10000张真实图片或视频，临时上传目录最终文件数为0。
- 按每批500条执行20批写入和20批更新。
- 验证100条分页、深页访问、响应体积限制、500条级联删除、WAL、外键和完整性。
- 本机实测约1.46秒完成10000商品元数据写入与整套校验，测试数据库小于64MB。
- 全部服务器测试：15/15通过。

### 24.4 前端和接口稳定性

- 用户商品接口继续限制分页，不会一次返回10000条。
- 管理后台改为服务端分页，每页100条。
- 搜索输入采用350毫秒防抖，避免每输入一个字都立即连续请求。
- 状态筛选、搜索、上一页、下一页全部由服务端执行。
- 原有不带 `with_meta=1` 的管理员商品接口仍返回数组，保持旧调用兼容。

### 24.5 硬盘保护

- 成功发布后自动只保留当前版本和最近6个回滚版本。
- 只在新版本健康、Nginx和COS步骤完成后清理旧发布。
- 自动清理未使用Docker构建缓存和悬空镜像。
- 不清理 `/srv/fuchong/shared/data`、`uploads`、`backups` 或生产环境文件。

---

## 25. 2026-07-30 管理后台批量磁盘删除、缓存上限与安全加固

### 25.1 管理后台入口

- “宠物商品管理”顶部新增醒目的“批量删除并释放磁盘”按钮，不再要求管理员横向滚动到每行最右侧。
- 表头支持“全选本页”，服务端分页每页最多100件，因此一次批量删除最多100件。
- 批量操作严格分为两步：
  1. 只读预检：返回可彻底删除数量、因订单等引用只能归档数量、本地媒体候选文件数和预计字节数。
  2. 明确确认：只有携带固定确认口令的第二次请求才执行。
- 有订单、支付、物流、售后或未知业务引用的商品只会安全归档，不删除商品、媒体或历史业务数据。
- 无业务阻断的商品才会事务性删除；事务提交后仅回收未被其他数据引用的本地普通文件。
- 接口：`POST /api/admin/pets/bulk-purge`。
- 接口只接受管理员 Bearer Token、`application/json`、1至100个去重后的正整数商品ID；独立限制为每个真实客户端IP十分钟最多6次。
- 全部执行结果写入 `product_deletion_jobs`、`media_deletion_queue` 和 `admin_operation_logs`。

### 25.2 一万件商品真实容量估算

生产库布偶猫飞书同步样本的只读抽样结果：

- 原图3份：11,749、13,756、11,577字节，平均约12.36KB/件。
- 原视频3份：1,684,999、2,280,740、1,508,348字节，平均约1.825MB/件。
- 服务器H.264兼容视频缓存3份：合计4,917,375字节，平均约1.639MB/件。
- 白底橱窗图缓存当前平均约5.9KB/文件。
- 飞书图片原图/缩略缓存当前平均约8.6KB/文件。

按“每件1张图+1段与当前布偶猫相近的视频”计算：

- 原始媒体约1.837MB/件；10,000件约18.37GB（约17.11GiB）。
- 若10,000段视频全部被访问并在服务器生成兼容缓存，额外约16.39GB（约15.27GiB）。
- 图片、白底图、缩略图、数据库文字和索引相对较小；10,000件元数据压力测试数据库小于64MB。
- 不设缓存上限时，原始媒体与兼容缓存完全双份可能达到约32.6GiB，当前59GB系统盘不适合长期无限缓存。
- 若每件改为多图或更长视频，容量按媒体总字节线性增长；例如视频平均10MB时，仅10,000段原视频即约100GB。
- 推荐架构：原始和长期媒体放COS/CDN，服务器只保存数据库与有上限的可再生缓存，不把服务器系统盘当长期媒体仓库。

### 25.3 可再生缓存硬上限

- 仅对可以重新生成的三个缓存目录执行容量与时效清理，不触碰商品原始媒体、COS、数据库、订单或上传源文件。
- 兼容视频缓存上限：8GiB。
- 飞书图片缓存上限：1GiB。
- 白底橱窗图缓存上限：1GiB。
- 缓存文件默认30天未更新可清理；服务启动、每10分钟及媒体处理完成后会执行受节流的检查。
- 只删除缓存目录直属普通文件，拒绝目录和符号链接。
- 环境变量：
  - `COMPATIBLE_VIDEO_CACHE_MAX_BYTES=8589934592`
  - `FEISHU_IMAGE_CACHE_MAX_BYTES=1073741824`
  - `SHOWCASE_IMAGE_CACHE_MAX_BYTES=1073741824`

### 25.4 安全加固

- 保留现有HTTPS、HSTS、`nosniff`、禁止跨站嵌入、来源校验、管理员短期签名令牌、密码变更后旧令牌失效、Docker `no-new-privileges` 和全部 capability drop。
- 修正反向代理环境中的真实客户端IP识别，登录防爆破和管理接口限流不再把所有访客错误合并为同一个容器地址。
- `/uploads/*` 读取统一通过受管路径解析，拒绝目录穿越、上传根目录、非法编码和越界路径。
- 批量破坏性操作增加独立低频限流、JSON类型校验、数量上限、预检、二次确认和完整审计。
- 压力测试只生成10,000件商品、图片URL和视频URL元数据，不生成10,000张真实图片或视频。

### 25.5 生产发布与验收

- 生产版本：`20260730-bulk-purge-security-v1`
- 前端资源：`assets/index-C-UpgC13.js`
- 线上资源已确认包含：
  - `批量删除并释放磁盘`
  - `全选本页`
  - `正在进行只读安全预检`
- 容器：healthy。
- API健康：`{"ok":true,"database":true}`。
- 数据库：`integrity_check=ok`，外键异常0。
- 发布后核心计数：584用户、201商品、44订单、5支付、8物流事件、50客服会话、291消息。
- 删除审计表保持为0：发布过程没有执行任何商品删除。
- 磁盘：59GB总量、23GB已用、34GB可用、使用率41%。
- 共享目录：data约57MB、uploads约552KB、backups约391MB。
- 未登录批量删除请求返回401。
