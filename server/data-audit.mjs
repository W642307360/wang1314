...
const report = {
  ...
  merchant_product_breed_link_mismatches: scalar("SELECT COUNT(*) FROM pets p LEFT JOIN breeds b ON b.id=p.breed_id WHERE p.merchant_account_id IS NOT NULL AND (b.id IS NULL OR b.category_id<>p.category_id OR b.name<>p.breed)"),
  merchant_product_row_breed_mismatches: scalar("SELECT COUNT(*) FROM pets p JOIN pet_products pp ON pp.pet_id=p.id WHERE p.merchant_account_id IS NOT NULL AND COALESCE(pp.breed_id,-1)<>COALESCE(p.breed_id,-1)"),
  completed_sync_counter_mismatches: scalar("SELECT COUNT(*) FROM feishu_sync_tasks WHERE status IN ('completed','completed_with_warnings') AND (processed<>total OR success+failed<>processed)"),
  ...
  sync_media_counter_mismatches: scalar("SELECT COUNT(*) FROM feishu_sync_tasks t WHERE t.media_total<>(SELECT COUNT(*) FROM feishu_sync_task_items i WHERE i.task_id=t.id AND i.showcase_status<>'not_required') OR t.media_success<>(SELECT COUNT(*) FROM feishu_sync_task_items i WHERE i.task_id=t.id AND i.showcase_status='success') OR t.media_failed<>(SELECT COUNT(*) FROM feishu_sync_task_items i WHERE i.task_id=t.id AND i.showcase_status='failed') OR t.media_processed<>t.media_success+t.media_failed"),
  stale_showcase_processing_items: scalar("SELECT COUNT(*) FROM feishu_sync_task_items i JOIN feishu_sync_tasks t ON t.id=i.task_id WHERE i.showcase_status='processing' AND t.status<>'processing_images'"),
