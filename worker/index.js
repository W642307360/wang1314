...
async function listPets(db, url, admin = false) {
  ...
  const pageSize = Math.min(admin ? 500 : 200, Math.max(1, Number(url.searchParams.get("pageSize")) || (admin ? 100 : 12)));
...
async function handlePublic(request, env, url, path, method) {
  ...
  if (path === "/api/community-applications" && method === "POST") {
    const x = await body(request), applicationType = text(x.application_type);
    if (!["breed","adoption","charity"].includes(applicationType)) return error("申请类型不合法");
    const subject = text(x.subject).slice(0,80), applicantName = text(x.applicant_name).slice(0,40), contact = text(x.contact).slice(0,80), details = text(x.details).slice(0,2000);
    if (subject.length < 2 || applicantName.length < 2) return error("请填写完整的申请主题和姓名");
    if (contact.length < 5) return error("请填写可联系到您的手机号或微信号");
    if (details.length < 10) return error("申请说明至少需要10个字");
    const duplicate = await first(db, `SELECT id,application_no,status FROM community_applications WHERE application_type=? AND subject=? AND contact=? AND datetime(created_at)>=datetime('now','-2 minutes') ORDER BY id DESC LIMIT 1`, applicationType, subject, contact);
    if (duplicate) return json({ ...duplicate, message: "申请已经收到，请勿重复提交" });
    const applicationNo = `FC${new Date().toISOString().slice(0,10).replaceAll("-","")}${crypto.randomUUID().slice(0,8).replaceAll("-","").toUpperCase()}`;
    const userId = id(x.user_id); const existingUser = userId ? await first(db,"SELECT id FROM users WHERE id=?",userId) : null;
    const r = await run(db, `INSERT INTO community_applications(application_no,user_id,application_type,subject,applicant_name,contact,city,details,availability,experience,metadata_json,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,'pending')`, applicationNo, existingUser?.id || null, applicationType, subject, applicantName, contact, text(x.city).slice(0,60)||null, details, text(x.availability).slice(0,200)||null, text(x.experience).slice(0,1000)||null, JSON.stringify(x.metadata && typeof x.metadata === "object" ? x.metadata : {}));
    return json({ id:r.meta.last_row_id,application_no:applicationNo,status:"pending",message:"申请已提交，运营团队会在后台跟进" },201);
  }
  if (path === "/api/community-applications" && method === "GET") {
    const userId=id(url.searchParams.get("user_id")); if(!userId)return error("用户信息无效");
    return json(await all(db,"SELECT id,application_no,application_type,subject,status,admin_reply,created_at,updated_at FROM community_applications WHERE user_id=? ORDER BY id DESC LIMIT 100",userId));
  }
...
async function adminStats(db) {
  ...
  const operations = { pending_after_sales: await scalar("SELECT COUNT(*) value FROM after_sales WHERE status='pending'"), pending_complaints: await scalar("SELECT COUNT(*) value FROM complaints WHERE status='pending'"), pending_community_applications: await scalar("SELECT COUNT(*) value FROM community_applications WHERE status IN ('pending','processing')"), sync_errors: await scalar("SELECT COUNT(*) value FROM sync_task_errors") };
...
async function handleAdmin(request, env, url, path, method) {
  ...
  if (path === "/api/admin/pets" && method === "POST") {
    ...
    const petId=r.meta.last_row_id; const productStatus=text(x.product_status,x.status==="published"?"available":x.status==="sold"?"sold":"offline"); await db.batch([db.prepare("INSERT OR IGNORE INTO pet_products(pet_id,breed_id,seller_id,product_name,status) VALUES(?,?,?,?,?)").bind(petId,id(x.breed_id)||null,id(x.seller_id)||null,text(x.name),productStatus),db.prepare("INSERT OR IGNORE INTO inventory(pet_id,total_stock,locked_stock,available_stock,low_stock_threshold) VALUES(?,?,0,?,1)").bind(petId,Number(x.stock)||1,Number(x.stock)||1)]); return json(await petDetail(db,petId),201);
  }
  ...
  if(adminPet&&method==="PATCH"){
    ...
    const productStatus=x.product_status||(x.status?x.status==="published"?"available":x.status==="sold"?"sold":"offline":null);if(productStatus)await run(db,"INSERT INTO pet_products(pet_id,product_name,status) SELECT id,name,? FROM pets WHERE id=? ON CONFLICT(pet_id) DO UPDATE SET status=excluded.status,product_name=excluded.product_name,updated_at=CURRENT_TIMESTAMP",productStatus,petId); return json(await petDetail(db,petId));
  }
  if(adminPet&&method==="DELETE"){await run(db,"DELETE FROM pets WHERE id=?",id(adminPet[1]));return json({ok:true});}
  if(path==="/api/admin/pets/bulk-status"&&method==="PATCH"){const x=await body(request);const ids=(x.ids||[]).map(id).filter(Boolean),allProducts=x.scope==="all";if(!allProducts&&!ids.length)return error("请选择商品");const petStatus=text(x.status,"draft"),productStatus=text(x.product_status,petStatus==="published"?"available":petStatus==="sold"?"sold":"offline");let changed=0;if(allProducts){const result=await run(db,"UPDATE pets SET status=?,updated_at=CURRENT_TIMESTAMP WHERE status!='deleted'",petStatus);changed=result.meta.changes||0;await run(db,"UPDATE pet_products SET status=?,updated_at=CURRENT_TIMESTAMP WHERE pet_id IN (SELECT id FROM pets WHERE status!='deleted')",productStatus);}else{const result=await run(db,`UPDATE pets SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id IN (${placeholders(ids.length)})`,petStatus,...ids);changed=result.meta.changes||0;await run(db,`UPDATE pet_products SET status=?,updated_at=CURRENT_TIMESTAMP WHERE pet_id IN (${placeholders(ids.length)})`,productStatus,...ids);}return json({ok:true,changed,count:changed,product_status:productStatus});}
  ...
  const adminPayment=path.match(/^\/api\/admin\/orders\/(\d+)\/payment$/);
  if(adminPayment&&method==="POST"){const orderId=id(adminPayment[1]),order=await first(db,"SELECT * FROM orders WHERE id=?",orderId);if(!order)return error("订单不存在",404);if(order.payment_status==="paid")return json({...await orderPayload(db,orderId),idempotent:true});if(["cancelled","completed","after_sale"].includes(order.status)||order.payment_status==="refunded")return error("当前订单状态不能确认付款",409);const paymentNo=`MANUAL${Date.now()}${orderId}`,nextStatus=order.status==="pending_payment"?"pending_confirm":order.status;try{await db.batch([db.prepare("INSERT INTO payments(order_id,payment_no,channel,amount,status,paid_at,raw_payload) VALUES(?,?,'admin_manual',?,'paid',CURRENT_TIMESTAMP,?)").bind(orderId,paymentNo,order.total_amount,JSON.stringify({source:"admin_payment_confirm"})),db.prepare("UPDATE orders SET payment_status='paid',status=?,paid_at=COALESCE(paid_at,CURRENT_TIMESTAMP),updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(nextStatus,orderId),db.prepare("INSERT INTO order_status_history(order_id,from_status,to_status,operator_type,operator_id,note) SELECT ?,?,?,'admin',1,'管理员核实到账' WHERE ?!=?").bind(orderId,order.status,nextStatus,order.status,nextStatus)]);}catch(e){if(/busy|locked/i.test(String(e?.message||"")))return json({message:"订单付款正在同步，请稍后重试",retryable:true},503);throw e;}return json({...await orderPayload(db,orderId),payment_no:paymentNo,idempotent:false});}
  ...
  if(path==="/api/admin/community-applications"&&method==="GET")return json(await all(db,`SELECT ca.*,u.nickname user_nickname,u.phone user_phone FROM community_applications ca LEFT JOIN users u ON u.id=ca.user_id ORDER BY CASE ca.status WHEN 'pending' THEN 0 WHEN 'processing' THEN 1 WHEN 'approved' THEN 2 ELSE 3 END,ca.id DESC LIMIT 1000`));
  const communityApplication=path.match(/^\/api\/admin\/community-applications\/(\d+)$/);
  if(communityApplication&&method==="PATCH"){const x=await body(request),status=["pending","processing","approved","rejected","completed"].includes(x.status)?x.status:"processing";await run(db,"UPDATE community_applications SET status=?,admin_reply=?,assigned_admin_id=1,updated_at=CURRENT_TIMESTAMP WHERE id=?",status,text(x.admin_reply).slice(0,1000)||null,id(communityApplication[1]));return json(await first(db,"SELECT * FROM community_applications WHERE id=?",id(communityApplication[1])));}
  if(communityApplication&&method==="DELETE"){await run(db,"DELETE FROM community_applications WHERE id=?",id(communityApplication[1]));return json({ok:true});}
  ...
  if(path==="/api/admin/feishu/preview"&&method==="POST"){
    const x=await body(request),config=await first(db,"SELECT * FROM feishu_sync_configs WHERE id=?",id(x.config_id));if(!config)return error("同步配置不存在",404);
    const mapping=parse(config.field_mapping,{}),records=await feishuRecords(env,config);const items=records.map(record=>{const field=(key,fallback)=>feishuValue(record.fields?.[mapping[key]||fallback]);const fieldAny=(key,names)=>{for(const name of [mapping[key],...names].filter(Boolean)){const value=feishuValue(record.fields?.[name]);if(text(value))return value;}return"";};const media=(key,fallback)=>{const v=record.fields?.[mapping[key]||fallback];return(Array.isArray(v)?v:[]).map(i=>i.url||i.tmp_url||i.file_token).filter(Boolean);};const description=text(fieldAny("description",["详情介绍","商品详情","详细介绍","文字详情","商品介绍"]));const explicitBody=text(field("body_type","体型"));const classifyBody=value=>/(小型|迷你|小体|小体型|小型犬|小型猫)/.test(value)?"小型":/(大型|巨型|大体|大体型|大型犬|大型猫)/.test(value)?"大型":/(中型|中等|标准体型|中型犬|中型猫)/.test(value)?"中型":"";const body_type=classifyBody(explicitBody)||classifyBody(description)||"中型";return{external_id:record.record_id,name:text(field("name","商品名称"),`飞书商品${record.record_id.slice(-6)}`),breed:text(field("breed","品种"),"未分类"),seller_name:text(field("seller_name","商家名称"),"福宠认证宠物馆"),gender:text(field("gender","性别")),age_months:Number(field("age_months","年龄（月）"))||null,color:text(field("color","毛色")),body_type,personality:text(field("personality","性格")),health_status:text(field("health_status","健康状态"),"健康"),vaccine_record:text(field("vaccine_record","疫苗记录")),description,price:Number(field("price","价格"))||0,status:["上架","在售","published","available"].includes(text(field("status","商品状态")))?"published":"draft",images:media("images","主图文件"),videos:media("videos","视频文件"),stock:Math.max(0,Number(field("stock","库存"))||1),source:"feishu"};});
  ...
  if(commitPreview&&method==="POST"){
    const preview=await first(db,"SELECT * FROM feishu_sync_previews WHERE id=?",id(commitPreview[1]));if(!preview)return error("同步预览不存在",404);const options=await body(request),rawItems=parse(preview.items_json,[]),items=options.publish_after_sync?rawItems.map(item=>({...item,status:"published"})):rawItems;const task=await run(db,"INSERT INTO feishu_sync_tasks(config_id,mode,status,total,batch_size,processed,success,failed) VALUES(?,'incremental','running',?,100,0,0,0)",preview.config_id,items.length);const taskId=task.meta.last_row_id;let success=0,failed=0,rowNo=0;
